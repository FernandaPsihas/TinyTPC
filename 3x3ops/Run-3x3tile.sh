...
# being stored to /tinytpc/data/... and then copied to dunegpvm.
#
...
if [ "$#" -ne 7 ]; then
    printf 'ERROR! You must provide seven and only seven booleans ("true" or "false")!'
    exit 1
fi
...
if [  "$COPY_TO_DUNE" = false ];then
  echo "\n${log_directory} will NOT be copied to dunegpvm.\n"
else
  echo "\nNow copying ${log_directory} to dunegpvm.\n"
fi
