...
def plot_xy_and_key(df, date):
    ...
    plt.suptitle(f'Pedestal {date_str}')
...
def plot_adc_time(df, date = ''):
    ...
    for i in range(len(cids)):
        ...
        for ch in range(len(routed_v2a_channels)):
            ...
            ax[i%3][i//3].scatter(time, adc, s = 0.8, color=cm.plasma(weight), 
                               alpha = 0.9)
            ax[i%3][i//3].plot(time, adc, lw = 0.5, color=cm.plasma(weight), 
                               alpha = 0.9)
...
def main(filename):
    ...
    output_filename = f'pedestal_{date}.pdf'
