...
if [ "$#" -ne 5 ]; then
    printf 'ERROR! You must provide 4 booleans ("true" or "false") + 1 string!'
    exit 1
fi

# ARGS: RUN_BASE, RUN_PEDS, RUN_THRESHOLD, RUN_START_RUN
# f t t t
...
if [ "$4" = true ]; then
  ...
  if [ "$3" = false ]; then #the user wants to execute start_run_log_raw.py, but not threshold_qc.py  (so, there are no default output threshold_qc files)
    ...
    do
      ...
      if [ ${#input_config_files[@]} -ne 5 ]; then
      ...
      declare -a chip_ids_array=(13 23 24 33 34)
...
#CONFIG_FILENAME='/Users/tinytpc/GitCode/TinyTPC/3x3ops/net-configs/network-3x3-GT-1ch-v00.json'
CONFIG_FILENAME='/Users/tinytpc/GitCode/TinyTPC/3x3ops/net-configs/network-3x3-GT-9ch-v02.json'

#CONFIG_FILENAME='./controller/network-3x3-tile-short_byhand_v20-5chips.json'
...
do
  ...
  echo "$(sed '4q;d' base.log)"
  echo "$(sed '5q;d' base.log)"
...
do
  ...
  if [ "$(sed '15q;d' pedestal.log)" = '[FINISH BASE]' ];then
    echo '\nSuccess! \n'
  ...
  if (( $i_pedestal > 0 )); then
    ...
    for entry_recur in "$search_dir"/tile-id-3x3-recursive-pedestal*
    do
      if [ "$APP_KEYW" = "y" ]; then
  ...
  if [  "$RUN_THRESHOLD" = false ];then
    ...
    if [ "$dif_ped_files" = false ]; then
      DISABLED_FILE="$(ls -t ${log_directory}tile-id-3x3-pedestal-disabled-list*.json | head -n 1)"
      PEDESTAL_FILE="$(ls -t ${log_directory}tile-id-3x3-pedestal_*.h5| head -n 1)"
    ...
    echo "\nNow executing threshold_qc.py: python3 threshold_qc'${THRESHOLD_V}'.py --controller_config $CONFIG_FILENAME --disabled_list ${DISABLED_FILE}  --pedestal_file ${PEDESTAL_FILE} >> ./threshold.log  2>&1 \n"
  fi
...
do
  ...
  echo "\n------ Attempt: $i_threshold ------"
  echo 'python3 threshold_qc'${THRESHOLD_V}'.py --controller_config '${CONFIG_FILENAME}' --disabled_list '${DISABLED_FILE}'  --pedestal_file '${PEDESTAL_FILE}' >> threshold.log  2>&1'
  #python3 threshold_qc.py --controller_config ${CONFIG_FILENAME} --disabled_list ${DISABLED_FILE}  --pedestal_file ${PEDESTAL_FILE} --verbose>> threshold.log  2>&1
  python3 threshold_qc${THRESHOLD_V}.py --controller_config ${CONFIG_FILENAME} --disabled_list ${DISABLED_FILE}  --pedestal_file ${PEDESTAL_FILE} --verbose --cryo>> threshold.log  2>&1
  # --cryo

  if [ "$(sed '11q;d' threshold.log)" = '[FINISH BASE]' ];then
    echo '\nSuccess! \n'
  ...
  if (( $i_threshold > 0 )); then
    for i in {1..9}
    do
     CHIP_FILE="$(ls -t ./tile-id-3x3-config-1-*.json | head -n 1)"
     if [ "$APP_KEYW_THR" = "y" ]; then
       filename="${CHIP_FILE%.*}"
       mv ${CHIP_FILE} ${filename}_${KEYW_THR}${THRESHOLD_V}.json
       CHIP_FILE="${filename}_${KEYW_THR}${THRESHOLD_V}.json"
     fi
...
do
  ...
  if [ "$(sed '13q;d' start.log)" = '[FINISH BASE]' ];then
    echo '\nSuccess! \n'
